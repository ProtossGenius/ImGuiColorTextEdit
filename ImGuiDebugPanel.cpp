...
void TextEditor::ImGuiDebugPanel(const std::string &panelName) {
    ...
    if (ImGui::CollapsingHeader("Editor state info")) {
        ...
        for (int i = 0; i <= mState.mCurrentCursor; i++) {
            ...
            sanitizedStart =
                SanitizeCoordinates(mState.mCursors[ i ].mInteractiveStart);
            sanitizedEnd =
                SanitizeCoordinates(mState.mCursors[ i ].mInteractiveStart);
            ImGui::DragInt2("Interactive start",
                            &mState.mCursors[ i ].mInteractiveStart.mLine);
            ImGui::DragInt2("Interactive end",
                            &mState.mCursors[ i ].mInteractiveEnd.mLine);
            ImGui::Text("Sanitized start: %d, %d", sanitizedStart.mLine,
                        sanitizedStart.mColumn);
            ImGui::Text("Sanitized end:   %d, %d", sanitizedEnd.mLine,
                        sanitizedEnd.mColumn);
        }
    }
    ...
    if (ImGui::CollapsingHeader("Undo")) {
        ...
        numberOfRecordsText =
            "Number of records: " + std::to_string(mUndoBuffer.size());
        ...
        for (int i = 0; i < mUndoBuffer.size(); i++) {
            if (ImGui::CollapsingHeader(std::to_string(i).c_str())) {
                ...
                for (int j = 0; j < mUndoBuffer[ i ].mOperations.size(); j++) {
                    ImGui::Text(
                        "%s", mUndoBuffer[ i ].mOperations[ j ].mText.c_str());
                    ImGui::Text(mUndoBuffer[ i ].mOperations[ j ].mType ==
                                        UndoOperationType::Add
                                    ? "Add"
                                    : "Delete");
                    ImGui::DragInt2(
                        "Start",
                        &mUndoBuffer[ i ].mOperations[ j ].mStart.mLine);
                    ImGui::DragInt2(
                        "End", &mUndoBuffer[ i ].mOperations[ j ].mEnd.mLine);
                    ImGui::Separator();
                }
            }
        }
