...
static bool TokenizeLuaStyleString(const char *in_begin, const char *in_end,
                                   const char *&out_begin,
                                   const char *&out_end) {
    const char *p = in_begin;
    ...
    if (is_single_quote || is_double_quotes || is_double_square_brackets) {
        ...
        while (p < in_end) {
            ...
            if ((is_single_quote && *p == '\'') ||
                (is_double_quotes && *p == '"') ||
                (is_double_square_brackets && *p == ']' && p + 1 < in_end &&
                 *(p + 1) == ']')) {
                out_begin = in_begin;
            ...
            // handle escape character for "
            if (*p == '\\' && p + 1 < in_end &&
                (is_single_quote || is_double_quotes))
                p++;
...
static bool TokenizeLuaStyleIdentifier(const char *in_begin, const char *in_end,
                                       const char *&out_begin,
                                       const char *&out_end) {
    const char *p = in_begin;
...
static bool TokenizeLuaStyleNumber(const char *in_begin, const char *in_end,
                                   const char *&out_begin,
                                   const char *&out_end) {
    const char *p = in_begin;
...
static bool TokenizeLuaStylePunctuation(const char  *in_begin,
                                        const char  *in_end,
                                        const char *&out_begin,
                                        const char *&out_end) {
    (void) in_end;
...
std::unique_ptr<TextEditor::LanguageDefinition>
    TextEditor::LanguageDefinition::Cpp() {
    static bool inited  = false;
    auto        langDef = std::make_unique<LanguageDefinition>();
    if (!inited) {
    ...
    if (!inited) {
        ...
        langDef->mTokenize = [](const char *in_begin, const char *in_end,
            ...
            if (in_begin == in_end) {
                ...
                out_end      = in_end;
                paletteIndex = PaletteIndex::Default;
            } else if (TokenizeLuaStyleString(in_begin, in_end, out_begin,
                                              out_end))
                paletteIndex = PaletteIndex::String;
            else if (TokenizeLuaStyleIdentifier(in_begin, in_end, out_begin,
                                                out_end))
                paletteIndex = PaletteIndex::Identifier;
            else if (TokenizeLuaStyleNumber(in_begin, in_end, out_begin,
                                            out_end))
                paletteIndex = PaletteIndex::Number;
            else if (TokenizeLuaStylePunctuation(in_begin, in_end, out_begin,
                                                 out_end))
                paletteIndex = PaletteIndex::Punctuation;
...
std::unique_ptr<TextEditor::LanguageDefinition>
    TextEditor::LanguageDefinition::Cs() {
    static bool inited  = false;
    auto        langDef = std::make_unique<LanguageDefinition>();
    if (!inited) {
...
std::unique_ptr<TextEditor::LanguageDefinition>
    TextEditor::LanguageDefinition::Json() {
    static bool inited  = false;
    auto        langDef = std::make_unique<LanguageDefinition>();
    if (!inited) {
...
std::unique_ptr<TextEditor::LanguageDefinition>
    TextEditor::LanguageDefinition::BrainLuck() {
    static bool inited  = false;
    auto        langDef = std::make_unique<LanguageDefinition>();
    if (!inited) {
        static const char *const keywords[] = {
