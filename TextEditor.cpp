...
void TextEditor::SetTabSize(int aValue) { mTabSize = Max(1, Min(8, aValue)); }
...
void TextEditor::SelectAllOccurrencesOf(const char *aText, int aTextSize,
    ...
    while (true) {
        ...
        Coordinates lastAddedPos =
            mState.mCursors[ mState.GetLastAddedCursorIndex() ].mInteractiveEnd;
        if (lastAddedPos == startPos) break;
    }
}

bool TextEditor::AnyCursorHasSelection() const {
    for (int c = 0; c <= mState.mCurrentCursor; c++)
        if (mState.mCursors[ c ].HasSelection()) return true;
    return false;
}

bool TextEditor::AllCursorsHaveSelection() const {
    for (int c = 0; c <= mState.mCurrentCursor; c++)
        if (!mState.mCursors[ c ].HasSelection()) return false;
    return true;
}

void TextEditor::ClearExtraCursors() { mState.mCurrentCursor = 0; }
...
int TextEditor::GetFirstVisibleLine() { return mFirstVisibleLine; }

int TextEditor::GetLastVisibleLine() { return mLastVisibleLine; }
        ...
        for (int c = mState.mCurrentCursor; c > -1; c--) {
            ...
            if (canPasteToMultipleCursors) {
                ...
                u.mOperations.push_back({clipSubText, start,
                                         GetSanitizedCursorCoordinates(c),
    ...
    if (aLines.empty())
        ...
        for (size_t i = 0; i < aLines.size(); ++i) {
            ...
            for (size_t j = 0; j < aLine.size(); ++j)
                mLines[ i ].emplace_back(
                    Glyph(aLine[ j ], PaletteIndex::Default));
        }
    }
...
bool TextEditor::Render(const char *aTitle, bool aParentIsFocused,
    ...
    if (mCursorPositionChanged) OnCursorPositionChanged();
...
void TextEditor::EditorState::AddCursor() {
    // vector is never resized to smaller size, mCurrentCursor points to last
    // available cursor in vector
    mCurrentCursor++;
...
std::string TextEditor::GetSelectedText(int aCursor) const {
    if (aCursor == -1) aCursor = mState.mCurrentCursor;
...
bool TextEditor::Move(int &aLine, int &aCharIndex, bool aLeft,
    ...
    // invalid line
    if (aLine >= mLines.size()) return false;

    if (aLeft) {
        if (aCharIndex == 0) {
            if (aLockLine || aLine == 0) return false;
            aLine--;
    ...
    } else // right
    {
        if (aCharIndex == mLines[ aLine ].size()) {
            if (aLockLine || aLine == mLines.size() - 1) return false;
            aLine++;
            aCharIndex = 0;
        } else {
...
void TextEditor::MoveCoords(Coordinates &aCoords, MoveDirection aDirection,
    ...
    case MoveDirection::Left:
        if (charIndex == 0) {
            ...
            if (aWordMode) aCoords = FindWordStart(aCoords);
        }
        break;
...
void TextEditor::MoveHome(bool aSelect) {
    for (int c = 0; c <= mState.mCurrentCursor; c++)
        SetCursorPosition(
            Coordinates(mState.mCursors[ c ].mInteractiveEnd.mLine, 0), c,
            !aSelect);
}
...
void TextEditor::EnterCharacter(ImWchar aChar, bool aShift) {
              ...
              // time
    {
        ...
        if (aChar == '\n') {
            ...
            AddGlyphsToLine(coord.mLine + 1, newLine.size(),
...
void TextEditor::Backspace(bool aWordMode) {
    ...
    if (AnyCursorHasSelection())
        ...
        if (!AllCursorsHaveSelection()) // can't do backspace if any cursor at
                                        // {0,0}
        {
...
void TextEditor::Delete(bool aWordMode, const EditorState *aEditorState) {
    ...
    if (AnyCursorHasSelection()) {
        ...
        for (int c = mState.mCurrentCursor; c > -1; c--) {
            if (!mState.mCursors[ c ].HasSelection()) continue;
            u.mOperations.push_back({GetSelectedText(c),
                                     mState.mCursors[ c ].GetSelectionStart(),
                                     mState.mCursors[ c ].GetSelectionEnd(),
                                     UndoOperationType::Delete});
        ...
        if (!AllCursorsHaveSelection()) // can't do delete if any cursor at end
                                        // of last line
        {
...
void TextEditor::AddCursorForNextOccurrence(bool aCaseSensitive) {
    ...
    Coordinates nextStart, nextEnd;
    if (!FindNextOccurrence(selectionText.c_str(), selectionText.length(),
...
bool TextEditor::FindMatchingBracket(int aLine, int aCharIndex,
                                     Coordinates &out) {
    if (aLine > mLines.size() - 1) return false;
    int maxCharIndex = mLines[ aLine ].size() - 1;
    if (aCharIndex > maxCharIndex) return false;
    ...
    if (CLOSE_TO_OPEN_CHAR.find(mLines[ aLine ][ aCharIndex ].mChar) !=
        ...
        while (Move(currentLine, currentCharIndex, true)) {
            if (currentCharIndex < mLines[ currentLine ].size()) {
                char currentChar =
                    mLines[ currentLine ][ currentCharIndex ].mChar;
...
void TextEditor::ChangeCurrentLinesIndentation(bool aIncrease) {
    ...
    for (int c = mState.mCurrentCursor; c > -1; c--) {
        for (int currentLine = mState.mCursors[ c ].GetSelectionEnd().mLine;
            ...
            if (Coordinates{currentLine, 0} ==
                ...
                mState.mCursors[ c ].GetSelectionEnd() !=
                    mState.mCursors[ c ]
                        .GetSelectionStart()) // when selection ends at line
                                              // start
                continue;
    ...
    if (u.mOperations.size() > 0) AddUndo(u);
}
...
void TextEditor::MoveUpCurrentLines() {
    ...
    for (int c = mState.mCurrentCursor; c > -1; c--) {
        ...
        // no need to set mCursorPositionChanged as cursors will remain sorted
           ...
           GetLineMaxColumn(maxLine)}; // this line is swapped with line above,
                                       // need to find new max column
    u.mOperations.push_back(
...
void TextEditor::MoveDownCurrentLines() {
    ...
    for (int c = mState.mCurrentCursor; c > -1; c--) {
        ...
        // no need to set mCursorPositionChanged as cursors will remain sorted
           ...
           GetLineMaxColumn(maxLine + 1)}; // this line is swapped with line
                                           // below, need to find new max column
    u.mOperations.push_back(
        {GetText(start, end), start, end, UndoOperationType::Add});
    u.mAfter = mState;
    AddUndo(u);
...
void TextEditor::ToggleLineComment() {
    assert(!mReadOnly);
    if (mLanguageDefinition == nullptr) return;
        ...
        for (int currentLine = mState.mCursors[ c ].GetSelectionEnd().mLine;
            ...
            while (currentIndex < mLines[ currentLine ].size() &&
                   (mLines[ currentLine ][ currentIndex ].mChar == ' ' ||
                    mLines[ currentLine ][ currentIndex ].mChar == '\t'))
                currentIndex++;
            if (currentIndex == mLines[ currentLine ].size()) continue;
            int i = 0;
...
void TextEditor::RemoveCurrentLines() {
    ...
    for (int c = mState.mCurrentCursor; c > -1; c--) {
        ...
        if (mLines.size() > nextLine) // next line exists
        {
            ...
            SetCursorPosition({mState.mCursors[ c ].mInteractiveEnd.mLine, 0},
                              c);
...
void TextEditor::EnsureCursorVisible(int aCursor, bool aStartToo) {
    if (aCursor == -1) aCursor = mState.GetLastAddedCursorIndex();
...
TextEditor::Coordinates
    TextEditor::GetSanitizedCursorCoordinates(int aCursor, bool aStart) const {
...
TextEditor::Coordinates
    TextEditor::ScreenPosToCoordinates(const ImVec2 &aPosition,
                                       bool         *isOverLineNumber) const {
    ImVec2 origin = ImGui::GetCursorScreenPos();
    ImVec2 local(aPosition.x - origin.x + 3.0f, aPosition.y - origin.y);

    if (isOverLineNumber != nullptr) *isOverLineNumber = local.x < mTextStart;
...
TextEditor::Coordinates
    ...
    if (aFrom.mLine >= (int) mLines.size()) return aFrom;

    int   lineIndex = aFrom.mLine;
    auto &line      = mLines[ lineIndex ];
    int   charIndex = GetCharacterIndexL(aFrom);

    if (charIndex > (int) line.size() || line.size() == 0) return aFrom;
    if (charIndex == (int) line.size()) charIndex--;
    ...
    while (Move(lineIndex, charIndex, true, true)) {
        ...
        if (initialIsSpace && !isSpace || initialIsWordChar && !isWordChar ||
...
TextEditor::Coordinates
    ...
    if (charIndex >= (int) line.size()) return aFrom;

    bool initialIsWordChar = CharIsWordChar(line[ charIndex ].mChar);
    bool initialIsSpace    = isspace(line[ charIndex ].mChar);
    char initialChar       = line[ charIndex ].mChar;
    while (Move(lineIndex, charIndex, false, true)) {
        if (charIndex == line.size()) break;
        bool isWordChar = CharIsWordChar(line[ charIndex ].mChar);
        bool isSpace    = isspace(line[ charIndex ].mChar);
        if (initialIsSpace && !isSpace || initialIsWordChar && !isWordChar ||
            !initialIsWordChar && !initialIsSpace &&
...
int TextEditor::GetCharacterIndexL(const Coordinates &aCoords) const {
    if (aCoords.mLine >= mLines.size()) return -1;
    ...
    for (; i < line.size() && c < aCoords.mColumn;) {
        if (line[ i ].mChar == '\t') {
            if (tabCoordsLeft == 0) tabCoordsLeft = TabSizeAtColumn(c);
            if (tabCoordsLeft > 0) tabCoordsLeft--;
            c++;
        } else
            ++c;
        if (tabCoordsLeft == 0) i += UTF8CharLength(line[ i ].mChar);
    }
    return i;
}
...
void TextEditor::RemoveLines(int aStart, int aEnd) {
    ...
    for (int c = 0; c <= mState.mCurrentCursor; c++) {
        ...
        if (mState.mCursors[ c ].mInteractiveStart.mLine >= aStart) {
            int targetLine =
                mState.mCursors[ c ].mInteractiveStart.mLine - (aEnd - aStart);
            targetLine = targetLine < 0 ? 0 : targetLine;
...
void TextEditor::DeleteRange(const Coordinates &aStart,
    ...
    if (aStart.mLine == aEnd.mLine) {
        ...
        if (aStart.mLine < aEnd.mLine) {
            AddGlyphsToLine(aStart.mLine, firstLine.size(), lastLine.begin(),
                            lastLine.end());
            for (int c = 0; c <= mState.mCurrentCursor;
                 c++) // move up cursors in line that is being moved up
            {
                // if cursor is selecting the same range we are deleting, it's
                // because this is being called from DeleteSelection which
                // already sets the cursor position after the range is deleted
                if (mState.mCursors[ c ].GetSelectionStart() == aStart &&
                    mState.mCursors[ c ].GetSelectionEnd() == aEnd)
                    continue;
                if (mState.mCursors[ c ].mInteractiveEnd.mLine > aEnd.mLine)
                    break;
                else if (mState.mCursors[ c ].mInteractiveEnd.mLine !=
                         aEnd.mLine)
                    continue;
...
void TextEditor::HandleKeyboardInputs(bool aParentIsFocused) {
    if (ImGui::IsWindowFocused() || aParentIsFocused) {
        ...
        if (!mReadOnly && !io.InputQueueCharacters.empty() && !(ctrl && !alt) &&
            !super) // See
                    // https://github.com/santaclose/ImGuiColorTextEdit/pull/34
        {
    ...
    */
    ...
    if (ImGui::IsWindowHovered()) {
        ...
        if (!shift && !alt) {
            ...
            if (click) mDraggingSelection = true;
...
void TextEditor::UpdateViewVariables(float aScrollX, float aScrollY) {
    ...
    mLastVisibleColumn = Max(
        (int) ((mContentWidth + aScrollX - mTextStart) / mCharAdvance.x), 0);
}
...
void TextEditor::Render(bool aParentIsFocused) {
    ...
    if (mShowLineNumbers) {
        ...
        mTextStart += ImGui::GetFont()
                          ->CalcTextSizeA(ImGui::GetFontSize(), FLT_MAX, -1.0f,
                                          lineNumberBuffer, nullptr, nullptr)
                          .x;
    }
    ...
    if (mEnsureCursorVisible > -1) {
        for (int i = 0; i < (mEnsureCursorVisibleStartToo ? 2 : 1);
             i++) // first pass for interactive end and second pass for
                  // interactive start
        {
            if (i)
                UpdateViewVariables(mScrollX,
                                    mScrollY); // second pass depends on changes
                                               // made in first pass
            Coordinates targetCoords = GetSanitizedCursorCoordinates(
                mEnsureCursorVisible, i); // cursor selection end or start
            if (targetCoords.mLine <= mFirstVisibleLine) {
                float targetScroll = std::max(
                    0.0f, (targetCoords.mLine - 0.5f) * mCharAdvance.y);
                if (targetScroll < mScrollY) ImGui::SetScrollY(targetScroll);
            }
            if (targetCoords.mLine >= mLastVisibleLine) {
                float targetScroll = std::max(
                    0.0f, (targetCoords.mLine + 1.5f) * mCharAdvance.y -
                              mContentHeight);
                if (targetScroll > mScrollY) ImGui::SetScrollY(targetScroll);
            }
    ...
    if (mSetViewAtLine > -1) {
        ...
        case SetViewAtLineMode::LastVisibleLine:
            targetScroll =
                std::max(0.0f, (float) (mSetViewAtLine - (mLastVisibleLine -
                                                          mFirstVisibleLine)) *
                                   mCharAdvance.y);
            break;
...
void TextEditor::OnCursorPositionChanged() {
    if (mState.mCurrentCursor == 0 &&
        !mState.mCursors[ 0 ]
             .HasSelection()) // only one cursor without selection
...
void TextEditor::OnLineChanged(
    ...
    bool aDeleted) // adjusts cursor position when other cursor writes/deletes
                   // in the same line
{
    ...
    if (aBeforeChange) {
        ...
        for (int c = 0; c <= mState.mCurrentCursor; c++) {
            if (mState.mCursors[ c ].mInteractiveEnd.mLine ==
                ...
                mState.mCursors[ c ].GetSelectionEnd() ==
                    mState.mCursors[ c ]
                        .GetSelectionStart()) // cursor does not have a
                                              // selection
            {
            ...
            if (hasTokenizeResult == false) {
                ...
                if (token_color == PaletteIndex::Identifier) {
                    ...
                    if (!line[ first - bufferBegin ].mPreprocessor) {
                        ...
                        if (mLanguageDefinition->mPreprocIdentifiers.count(
                                id) != 0)
                            token_color = PaletteIndex::PreprocIdentifier;
                    }
                }
...
void TextEditor::ColorizeInternal() {
    if (mLines.empty() || mLanguageDefinition == nullptr) return;
